...
@app.route('/quiz_take/<string:ans>', methods = ['GET'])
def student_quiz_take_ans(ans):
    if 'user_id' in session:
        g.stu_name = session['username']
    if request.method == 'GET':
        sql = 'SELECT question, answer, mark, attempt FROM quiz where course=%s'
        data = (
            session['sub'],
        )
        cursor.execute(sql, data)
        rowExist = cursor.fetchone()
        if rowExist[1]==ans and not rowExist[3]:
            markUpdate = 'SELECT mark FROM user where id=%s'
            data = (
                session['user_id'],
            )
            cursor.execute(markUpdate, data)
            marks = cursor.fetchone()

            markUpdate = 'UPDATE user set mark=%s where id=%s'
            data = (
                (int)(marks[0] + rowExist[2]),
                session['user_id']
            )
            ...
            cursor.execute(markUpdate, data)
            conn.commit()
        markUpdate = 'UPDATE quiz set attempt=%s where answer=%s'
        data = (
            int(1),
            ans
        )
...
def quiz_take():
    if 'user_id' in session:
        g.stu_name = session['username']
    sql = 'SELECT question, o1, o2, o3, o4 from quiz where course=%s and attempt = 0'
    g.name = session['sub']
    data = (
        session['sub'],
    )
...
def submit():
    if 'user_id' in session:
        ...
        g.sub = session['sub']
    if request.method == 'POST':
        queryExist = 'SELECT question, answer from quiz where course = %s'
        try:
            cursor.execute(queryExist, (g.sub,))
            rowExist = cursor.fetchall() 
            print(rowExist)
            print(str(request.form['answer']))
            return redirect(url_for('student'))
        except Error as error:  
            print("Hello1")  
    return redirect(url_for('student'))
...
def add():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(rowExist)
            if not rowExist:
                sql = "INSERT INTO quiz (course, question, o1, o2, o3, o4, answer, mark, attempt) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
                print(sql)
                data = (
